...
from mailconfig import get_mail_quota, set_mail_quota, get_default_quota, validate_quota, set_mail_dailylimit
...
def mail_users_add():
	...
	limit = request.form.get('dailylimit', 0)
	try:
		return add_mail_user(request.form.get('email', ''), request.form.get('password', ''), request.form.get('privileges', ''), quota, limit, env)
	except ValueError as e:
		return (str(e), 400)
...
@app.route('/mail/users/dailylimit', methods=['POST'])
@authorized_personnel_only
def mail_users_dailylimit():
	try:
		return set_mail_dailylimit(request.form.get('email', ''), request.form.get('dailylimit'), env)
	except ValueError as e:
		return (str(e), 400)
