...
if [ ! -f "$db_path" ]; then
	...
	echo "CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT NOT NULL UNIQUE, password TEXT NOT NULL, extra, privileges TEXT NOT NULL DEFAULT '', quota TEXT NOT NULL DEFAULT '0', daily_limit INTEGER NOT NULL DEFAULT 10, emails_sent INTEGER NOT NULL DEFAULT 0, last_reset TEXT DEFAULT '');" | sqlite3 "$db_path";
...
user_query = SELECT email AS user, "mail" as uid, "mail" as gid, "$STORAGE_ROOT/mail/mailboxes/%d/%n" as home, '*:bytes=' || quota AS quota_rule, daily_limit, emails_sent, last_reset FROM users WHERE email='%u';
